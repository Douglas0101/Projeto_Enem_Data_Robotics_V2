...
import type { TbNotasGeoUfRow } from "../types/dashboard";
import {
  DashboardCard,
...
interface Props {
  rows: TbNotasGeoUfRow[];
  isLoading?: boolean;
}

/**
 * Componente de tabela para Detalhamento Estadual (por UF).
 * Exibe médias de notas agregadas por estado/UF.
 */
export function NotasGeoUfTable({ rows, isLoading }: Props) {
  ...
  const handleExportCsv = () => {
    ...
    link.href = url;
    link.download = `detalhamento-estadual-${rows[0]?.ANO ?? "dados"}.csv`;
    link.click();
  ...
  const handleExportPdf = () => {
      ...
      .map(
        (row) => `
          <tr>
            ...
            <td>${escapeHtml(row.INSCRITOS?.toLocaleString("pt-BR"))}</td>
    ...
    popup.document.write(
      ...
      <html>
        <head>
          <meta charset="utf-8" />
          <title>Detalhamento por Estado</title>
          <style>${styles}</style>
        </head>
        <body>
      ...
      <DashboardCardHeader className="flex-row items-center justify-between space-y-0 pb-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">
            Médias de notas do ENEM agregadas por unidade federativa (UF)
          </p>
        </div>
        <div className="flex items-center gap-2">
        ...
        {!rows.length ? (
          ...
          <div className="h-full w-full overflow-auto">
            <Table>
              ...
              <TableBody>
                {rows.map((row) => (
                  <TableRow key={`${row.ANO}-${row.SG_UF_PROVA}`}>
                    <TableCell className="font-medium">{row.ANO}</TableCell>
                    <TableCell className="font-semibold">{row.SG_UF_PROVA}</TableCell>
                    <TableCell className="text-right text-muted-foreground">
                      {row.INSCRITOS?.toLocaleString("pt-BR") ?? "–"}
                    </TableCell>
                    <TableCell className="text-right text-muted-foreground">
