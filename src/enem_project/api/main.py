"""
Módulo principal da API FastAPI do ENEM Data Robotics.

Este módulo configura a aplicação FastAPI, incluindo:
- Ciclo de vida (startup/shutdown) com materialização de tabelas
- Middlewares (CORS, Rate Limiting, Request ID)
- Routers (Dashboard, Chat, Auth)
...
from ..orchestrator.workflows.sql_backend_workflow import (
...
async def lifespan(_app: FastAPI):  # pylint: disable=unused-argument
    ...
    if not db_path.exists():
        ...
        try:
            ...
            table_check = conn.execute(
                ...
                "WHERE table_name = 'tb_notas_stats'"
            ).fetchone()
            conn.close()

            if table_check and table_check[0] == 0:
                logger.warning(
                    "Tabela crítica 'tb_notas_stats' ausente. "
    ...
    if should_materialize:
        ...
        try:
            ...
            # which is synchronous. We run it as is during startup.
...
app = FastAPI(
    ...
    version="1.0.0",
    description=(
        "API analítica profissional refatorada para estabilidade e performance."
    ),
    lifespan=lifespan,
)
...
async def global_exception_handler(request: Request, exc: Exception):
    """Trata exceções não capturadas retornando resposta JSON padronizada."""
