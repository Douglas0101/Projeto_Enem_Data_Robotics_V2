...
def run_soda_scan(conn: DuckDBPyConnection) -> int:
    ...
    if not _SODA_AVAILABLE:
        logger.warning("Soda Core não instalado; pulando scan de qualidade (use poetry add soda-core para habilitar).")
        return 0
    ...
    if not config_path.exists() or not checks_path.exists():
        logger.error(f"Arquivos de configuração do Soda não encontrados: {config_path} ou {checks_path}")
        return 1
...
def assert_dashboard_data_checks(conn: DuckDBPyConnection) -> None:
    ...
    if exit_code > 0:
        raise RuntimeError(f"Soda Data Quality Check falhou com código {exit_code}. Verifique os logs.")
