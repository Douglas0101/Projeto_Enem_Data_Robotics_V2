...
def _detect_ram_gb() -> float:
    ...
    return 20.0  # default razoável pro seu caso
...
def _detect_available_ram_gb(total: float) -> float:
    if psutil is not None:
        try:
            ...
            pass
    # fallback: assume ~80% disponível se não souber calcular
    return total * 0.8
...
def _resolve_max_ram_pipeline(ram_gb: float) -> float:
    ...
    # Nunca usar mais que ~60% da RAM total para um único pipeline pesado,
    # limitado a 12 GB por segurança em máquinas com ~20 GB.
...
def _calculate_chunk_rows(max_ram_pipeline: float) -> int:
    ...
    # Aproximação: reservar 25% do orçamento de RAM do pipeline para cada chunk
    # e assumir um custo médio de 450 bytes por linha.
...
def _resolve_streaming_threshold(
    ...
    # Limite inferior de 1.5 GB evita streaming desnecessário em arquivos pequenos
    return max(1.5, base)
